...
- **`chargeMiscFee` is unbounded, but not directly withdrawable.** It credits the staging pool `ACC_UNALLOCATED_BORROWER_MISC_FEE_PAYABLE` and debits `ACC_BORROWER_MISC_FEE_RECEIVABLE` for any caller-supplied amount, with no cap. The withdrawable `ACC_SERVICER_MISC_FEE_PAYABLE` is created only in `applyWaterfall`, capped at cash the borrower actually paid — so an uncollected charge is not, by itself, extractable.
- **Siphoning a fresh borrower payment.** After `receiveBorrowerPayment(1000)` the 1000 sits in `ACC_BORROWER_PAYMENT_CLEARING`. A servicer can call `chargeMiscFee(1000)` then `applyWaterfall(miscFees=1000, …)` — the waterfall's misc-fee cap (`miscFees ≤ ACC_BORROWER_MISC_FEE_RECEIVABLE + ACC_BORROWER_MISC_FEE_PAID`) was just inflated by `chargeMiscFee`, so it passes — followed by `servicerWithdraw` to take the 1000 from `ACC_CASH`. The borrower's interest/principal never get allocated. This diverts a fresh payment the borrower actually made; it cannot reach cash already allocated to the investor.
...
When the servicer diverts a fresh borrower payment to fees, the per-loan cash check at [`LoansLedger.sol:127`](contracts/LoansLedger.sol#L127) means `servicerWithdraw`, `investorWithdraw`, and `originatorWithdraw` are competing for the same `ACC_CASH` pool — whoever transacts first wins. Under standard flows this race is invisible because every withdrawable payable is cash-backed (interest/servicer/investor legs are capped at `-ACC_BORROWER_PAYMENT_CLEARING`, the origination fee is withheld from disbursed cash, and the servicer misc-fee payable is promoted from the unallocated pool only in `applyWaterfall`), so the sum of positive net payables never exceeds `ACC_CASH`. The one path that can still break that equality is the freeform `createLedgerEntries` adjustment primitive, which can mint a withdrawable payable with no matching cash — a trusted-servicer (T-4) action, not an honest-flow condition.
