...
- **[E]** Pause discipline: own OZ `Pausable` state (independent of `Loans`' — pausing one does not pause the other), auth resolved against `Loans` roles (admin/guardian/pauser can `pause`, guardian-only `unpause`). While paused, `transferFrom`/`safeTransferFrom`/`lock` revert `EnforcedPause`; `approve`/`setApprovalForAll` revert only when _granting_ — revocations (`approve(address(0))`, `setApprovalForAll(…, false)`) stay live so owners can strip compromised operators mid-incident; `unlock` and `forceTransfer` deliberately stay live so incident recovery (`forceCancelOffer`, guardian rescue) keeps working.
